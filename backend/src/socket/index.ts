...
import { getConfig, getSocketConfig, getCacheConfig } from '../lib/config.js'
    ...
    if (typingEntry?.userId === user.id) {
      ...
      if (isAdmin({ role: user.role })) {
        ...
        db.query.conversations.findFirst({
          ...
          getOnlineSuperAdminIds().forEach(adminId => {
            if (adminId !== conv?.assignedAdminId) emitToUser(adminId, 'typing:stop', stopPayload)
          })
        }).catch(() => { /* best-effort */ })
      }
    }
...
async function handleMessageSend(socket: Socket, user: SocketUser, data: unknown, clientIp: string): Promise<void> {
  ...
  if (isAdmin({ role: user.role })) {
    ...
    const onlineSuperAdmins = getOnlineSuperAdminIds()
    onlineSuperAdmins.forEach(adminId => {
      if (adminId !== user.id) {
        emitToUser(adminId, 'message:new', { message: messagePayload })
        emitToUser(adminId, 'conversation:updated', updatePayload)
      }
    })
  } else {
    ...
    // Emit to SUPER_ADMINs for oversight — collect once, reuse for conversation:updated below
    const onlineSuperAdmins = getOnlineSuperAdminIds()
    onlineSuperAdmins.forEach(adminId => {
      if (adminId !== conversation.assignedAdminId) {
        emitToUser(adminId, 'message:new', { message: messagePayload })
        emitToUser(adminId, 'conversation:updated', updatePayload)
...
async function handleTypingStart(socket: Socket, user: SocketUser, providedConvId?: string): Promise<void> {
  ...
  if (isAdmin({ role: user.role })) {
    ...
    // User typing → send to the assigned admin + all online SUPER_ADMINs.
    // Collect super-admin IDs once — avoids scanning connectedUsers on every keystroke.
    const superAdminIds = getOnlineSuperAdminIds()
    if (conversation.assignedAdminId) {
    ...
    for (const adminId of superAdminIds) {
      if (adminId !== conversation.assignedAdminId) {
        emitToUser(adminId, 'typing:start', { conversationId, userId: user.id, userName })
      }
    }
  }
...
async function handleTypingStop(socket: Socket, user: SocketUser, providedConvId?: string): Promise<void> {
  ...
  if (isAdmin({ role: user.role })) {
    ...
    for (const adminId of superAdminIds) {
      if (adminId !== conversation.assignedAdminId) {
        emitToUser(adminId, 'typing:stop', { conversationId, userId: user.id, userName })
      }
    }
  }
...
/** Returns the user IDs of all currently-connected SUPER_ADMINs — call once per message, not twice. */
export function getOnlineSuperAdminIds(): string[] {
  const ids: string[] = []
  for (const [userId] of serverState.connectedUsers.entries()) {
    if (getUserFromCache(userId)?.role === 'SUPER_ADMIN') ids.push(userId)
  }
  return ids
}
