...
import { getConfig, getSessionConfig, getSecurityConfig } from '../lib/config.js'
import {
  getUserFromCache,
  addUserToCache,
...
export function signToken(payload: {
  ...
  return jwt.sign(
    ...
    env.jwtSecret,
    {
      expiresIn: `${env.jwtExpiryMinutes}m`,
...
async function validateSessionWithUser(userId: string, sessionId: string): Promise<SessionWithUser> {
  ...
  setSessionValidationCache(userId, sessionId, valid, sessionConfig.validationTTLMs)
...
/**
 * Resolves the correct SameSite + Secure policy for cookies based on the
 * deployment topology:
 *
 *   local dev   (localhost)          → lax  / secure: false
 *   same-origin prod (same hostname) → strict / secure: true
 *   split-origin prod (diff hosts)   → none  / secure: true  (required by spec)
 *
 * "Split-origin" means the frontend and backend are on different hostnames,
 * e.g. frontend on *.pages.dev and backend on *.koyeb.app.  In that case
 * SameSite=Strict or Lax causes the browser to silently drop all cookies on
 * every cross-origin fetch, so we must use None (which requires Secure=true).
 */
function resolveCookiePolicy(): { sameSite: 'lax' | 'strict' | 'none'; secure: boolean } {
  ...
  try {
    ...
    const issuerHost = new URL(env.jwtIssuer).hostname
    if (appHost === issuerHost) {
      return { sameSite: 'strict', secure: true }
    }
  } catch {
    // Malformed URLs — fall back to the safe cross-origin policy
  }

  // Different hostnames: cross-origin deployment, must use None+Secure
...
export function issueAuthCookies(reply: FastifyReply, token: string, refreshToken?: string): CsrfTokens {
  ...
  reply.setCookie('token', token, {
    ...
    sameSite,
    path: '/',
    maxAge: env.jwtExpiryMinutes * 60
  })
  ...
  // Bug 3 fix: tie CSRF cookie lifetime to the refresh token, not the access
  // token.  The access token expires in minutes; the refresh token lives for
  // days.  After a silent refresh the CSRF cookie must still be alive or every
  // subsequent mutation will 403.
  reply.setCookie(CSRF_COOKIE, csrfToken, {
    httpOnly: false, // intentionally readable by JS so the frontend can send it as a header
    secure,
    sameSite,
    path: '/',
    maxAge: config.session.refreshTokenDays * 24 * 60 * 60
...
export function validateCsrf(request: FastifyRequest): boolean {
  ...
  try {
    ...
    if (cookieBuf.length !== headerBuf.length) {
      logger.warn({ requestId: request.requestId }, 'CSRF validation failed: length mismatch')
      return false
    }

    return timingSafeEqual(cookieBuf, headerBuf)
  } catch (err) {
    logger.warn({ requestId: request.requestId, error: err }, 'CSRF validation error')
    return false
  }
}
...
export function clearAuthCookies(reply: FastifyReply): void {
  const { sameSite, secure } = resolveCookiePolicy()
  reply.clearCookie('token', { path: '/', secure, sameSite })
  reply.clearCookie('refresh_token', { path: '/api/auth/refresh', secure, sameSite })
  reply.clearCookie(CSRF_COOKIE, { path: '/', secure, sameSite })
}
