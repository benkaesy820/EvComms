...
export function createRateLimiters(): RateLimiters {
  ...
  return {
    ...
    socketMessage: (userId: string) => {
      const config = getConfig()
      const minuteResult = messageMinuteStore.increment(userId, 60 * 1000, config.limits.message.perMinute)
      const hourResult = messageHourStore.increment(userId, 60 * 60 * 1000, config.limits.message.perHour)
