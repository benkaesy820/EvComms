...
fastify.post('/register', { preHandler: rateLimiters.registration }, async (request, reply) => {
  ...
  if (lockout.locked) {
    ...
    // Enforce max devices from config
    const loginConfig = getConfig()
    await evictOldestSession(user.id, loginConfig.session.maxDevices, request.ip)
  ...
  fastify.post('/password/forgot', { preHandler: rateLimiters.passwordReset }, async (request, reply) => {
    ...
    if (user) {
      ...
      try {
        queueHighPriorityEmail(user.id, 'passwordReset', { resetToken: rawToken })
      } catch (error) {
