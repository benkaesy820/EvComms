...
export async function getAdminWorkloads(
  ...
  return adminIds.map((id) => {
    ...
    return {
      ...
      isOnline: serverState.isUserConnected(id),
...
export async function pickBestAdmin(subsidiaryId?: string | null): Promise<string | null> {
  ...
  // ── Step 1: collect online admin IDs from memory ──────────────────────────
  ...
  for (const [userId] of serverState.connectedUsers.entries()) {
    ...
    if (!cached || cached.status !== 'APPROVED') continue
    if (cached.role === 'ADMIN') onlineAdminIds.push(userId)
    else if (cached.role === 'SUPER_ADMIN') onlineSuperAdminIds.push(userId)
  }

  const allOnlineIds = [...onlineAdminIds, ...onlineSuperAdminIds]

  if (allOnlineIds.length > 0) {
    // Single DB query covers all online admins
    const workloads = await getAdminWorkloads(allOnlineIds)
  ...
  const workloads = await getAdminWorkloads(offlineIds, undefined, offlineSubMap)
...
export async function reassignForSubsidiary(
  ...
  const convUser = await db.query.users.findFirst({
  ...
  // Try the in-process cache first to avoid an extra DB round-trip.
  ...
  if (cachedNewAdmin) {
    ...
    const dbNewAdmin = await db.query.users.findFirst({
      where: eq(users.id, newAdminId),
      columns: { name: true, role: true },
    })
    newAdminName = dbNewAdmin?.name
    newAdminRole = dbNewAdmin?.role
  }
...
export async function getAllAdminWorkloads(): Promise<AdminWorkload[]> {
  ...
  return getAdminWorkloads(ids, nameMap)
}
