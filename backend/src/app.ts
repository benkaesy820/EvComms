...
fastify.addHook('onRequest', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.addContentTypeParser('application/json', { parseAs: 'string', bodyLimit: config.server?.jsonBodyLimit ?? 1048576 }, (request: FastifyRequest, body: string, done: (err: Error | null, body?: unknown) => void) => {
    try {
