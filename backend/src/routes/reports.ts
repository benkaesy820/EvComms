...
import { z } from 'zod'
import { desc, eq, and, lt, sql, inArray, or, isNull } from 'drizzle-orm'
...
export async function reportsRoutes(fastify: FastifyInstance) {
    ...
    fastify.get('/:id', { preHandler: requireAdmin }, async (request, reply) => {
        ...
        if (user.role === 'ADMIN') {
            // Run assignment check and existence check in parallel — they are independent
            const [assignedConv, anyConversation] = await Promise.all([
            ...
            if (!assignedConv) {
                // Check if PENDING user with no conversation at all
            ...
            // Reuse anyConversation result — no extra query needed
            return sendOk(reply, { report: normalizeReport(report), hasConversation: !!anyConversation })
        }

        // SUPER_ADMIN path: just check existingConv
