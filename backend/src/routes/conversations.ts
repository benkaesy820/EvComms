    ...
    if (userIsAdmin) {
      ...
      const rows = await db.all(sql`
        SELECT
          ...
          CASE WHEN m.id IS NOT NULL THEN json_object('id', m.id, 'type', CASE WHEN m.deleted_at IS NOT NULL THEN 'DELETED' ELSE m.type END, 'content', CASE WHEN m.deleted_at IS NOT NULL THEN NULL ELSE m.content END, 'sender_id', m.sender_id, 'created_at', m.created_at, 'deleted_at', m.deleted_at) ELSE NULL END as last_message
  ...
  fastify.post('/', { preHandler: requireApprovedUser }, async (request, reply) => {
    ...
    if (existing) {
      if (body.success && body.data.subsidiaryId !== undefined && existing.subsidiaryId !== body.data.subsidiaryId) {
        ...
        // Serverless Fastify FIX: Await the background assignment sequentially. 
        // Vercel/PaaS containers freeze the moment sendOk() replies, which aborts floating promises.
        // It takes ~15ms, completely safe to await inline.
        try {
          const { reassignForSubsidiary } = await import('../lib/assignmentEngine.js')
          await reassignForSubsidiary(
            existing.id,
          ...
          logger.warn({ err, conversationId: existing.id }, 'Subsidiary reassignment failed — non-fatal')
        }
    ...
    const normalizedConversation = conversation ? {
      ...
      deletedAt: conversation.deletedAt?.getTime() ?? null,
      waitingSince: conversation.waitingSince?.getTime() ?? null,
      lastAdminReplyAt: conversation.lastAdminReplyAt?.getTime() ?? null,
    } : null

    return reply.code(201).send({ success: true, conversation: normalizedConversation })
  })
...
fastify.post('/for-user', { preHandler: requireApprovedUser }, async (request, reply) => {
  ...
  try {
    ...
    if (isDbConstraintError(error)) {
      // Race condition: another concurrent request already inserted — return existing row silently
      const conv = await db.query.conversations.findFirst({ where: eq(conversations.userId, userId), columns: { id: true } })
      if (conv) {
        logger.debug({ adminId: user.id, targetUserId: userId }, 'Conversation insert race — returning existing conversation')
        return sendOk(reply, { conversation: conv })
      }
    }
    logger.error({ adminId: user.id, targetUserId: userId, error }, 'Conversation initiation failed')
    return sendError(reply, 500, 'INTERNAL_ERROR', 'Failed to initiate conversation')
  }

  // If onConflictDoNothing silently skipped the insert (row already existed), return existing
  const inserted = await db.query.conversations.findFirst({
    where: eq(conversations.id, id),
    columns: { id: true }
  })
  if (!inserted) {
    const existing = await db.query.conversations.findFirst({ where: eq(conversations.userId, userId), columns: { id: true } })
    if (existing) {
      logger.debug({ adminId: user.id, targetUserId: userId }, 'Conversation already exists — returning existing conversation')
      return sendOk(reply, { conversation: existing })
    }
  }
  ...
  const targetUserInfoRaw = await db.query.users.findFirst({
    where: eq(users.id, userId),
    columns: { id: true, name: true, email: true, status: true }
  })
  
  const targetUserInfo = targetUserInfoRaw ?? { id: userId, name: 'Unknown', email: '', status: 'APPROVED' }
  ...
  fastify.post('/:conversationId/messages', {
    ...
    const conversation = await db.query.conversations.findFirst({
      where: eq(conversations.id, conversationId),
      columns: { id: true, userId: true, assignedAdminId: true, unreadCount: true, adminUnreadCount: true, lastMessageAt: true, createdAt: true, subsidiaryId: true, archivedAt: true }
    })
    ...
    if (conversation.archivedAt) {
      return sendError(reply, 400, 'BAD_REQUEST', 'Cannot send messages to an archived conversation')
    }
    ...
    // Re-read conversation + optional snippets in parallel — saves up to 2 round-trips per message
    const [updatedConversation, replyToSnippetRaw, linkedAnnouncement] = await Promise.all([
      db.query.conversations.findFirst({
        where: eq(conversations.id, conversationId),
        columns: { unreadCount: true, adminUnreadCount: true, assignedAdminId: true }
      }),
      body.data.replyToId && isValidId(body.data.replyToId)
      ...
      body.data.announcementId && isValidId(body.data.announcementId)
    ...
    // Normalise replyTo: hide content from soft-deleted messages
    const replyToSnippet = replyToSnippetRaw
      ? { ...replyToSnippetRaw, content: replyToSnippetRaw.deletedAt ? null : replyToSnippetRaw.content, sender: replyToSnippetRaw.sender ?? null }
      : null
    ...
    if (userIsAdmin) {
      ...
      emitToSuperAdmins('conversation:updated', convUpdatePayload, user.id)
    } else {
      ...
      emitToSuperAdmins('message:new', { message: messagePayload }, assignedAdminId || undefined)
      emitToSuperAdmins('conversation:updated', convUpdatePayload, assignedAdminId || undefined)
...
const reactionSchema = z.object({
  // Normalise to NFC so encoding-equivalent emoji (e.g. composed vs decomposed)
  // always resolve to the same codepoint sequence before hitting the DB.
  emoji: z.string().min(1).max(10)
    .transform(s => s.normalize('NFC'))
    .pipe(z.string().regex(/^[\p{Emoji}]+$/u, 'Must be valid emoji'))
})
...
export async function reactionRoutes(fastify: FastifyInstance) {
  fastify.post('/:messageId/reactions', { preHandler: requireApprovedUser }, async (request, reply) => {
    ...
    try {
      ...
      // NOTE: the unique constraint must be on (messageId, userId) only — NOT
      // including emoji — otherwise libsql raises SQLITE_UNKNOWN instead of
      // triggering the DO UPDATE clause. See migration 0003.
      ...
      // LibSQL wraps the native SQLite error under `cause.code`; the top-level
      // `code` may be absent. Treat any constraint violation as a no-op success.
  ...
  fastify.delete('/:messageId/reactions/:emoji', { preHandler: requireApprovedUser }, async (request, reply) => {
    ...
    // conversation access check and reaction lookup are independent — run in parallel
    const [conversation, reaction] = await Promise.all([
      db.query.conversations.findFirst({
        where: eq(conversations.id, message.conversationId),
        columns: { id: true, userId: true, assignedAdminId: true }
      }),
      db.query.messageReactions.findFirst({
        where: and(
    ...
    // reaction already fetched in parallel above — remove the sequential re-fetch
    if (!reaction) {
