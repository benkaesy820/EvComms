...
import { z } from 'zod'
import { desc, eq, and, isNull, lt, sql, inArray, not } from 'drizzle-orm'
...
// Zod schema used for reaction body — normalises to NFC so encoding-equivalent
// emoji always produce the same stored value.
...
export async function internalRoutes(fastify: FastifyInstance) {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (otherAdmins.length > 0) {
        const now = new Date()
        await tx.insert(internalMessageReads)
          .values(otherAdmins.map(a => ({
            id: ulid(),
            userId: a.id,
            lastReadAt: now,
            unreadCount: 1,
          ...
          .onConflictDoUpdate({
            target: internalMessageReads.userId,
            set: {
              unreadCount: sql`${internalMessageReads.unreadCount} + 1`,
              updatedAt: now,
            }
          })
      }
  ...
  fastify.delete('/clear', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    if (user.role === 'SUPER_ADMIN') {
      // H-3 FIX: Wrap soft-delete + audit log in one transaction — crash between the two
      // previously left all messages deleted with no audit record.
      await db.transaction(async (tx) => {
        await tx.update(internalMessages)
  ...
  fastify.post('/:id/reaction', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    // Validate + NFC-normalise emoji via Zod (replaces unsafe type assertion)
    ...
    try {
      ...
      // NOTE: unique constraint is on (messageId, userId) only — see migration 0003.
  ...
  fastify.delete('/:id/reaction/:emoji', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    // Fastify params are sometimes URL encoded; normalise to NFC to match stored value
  ...
  fastify.post('/bulk-delete', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    // H-7 FIX: Pre-fetch all message rows in one query instead of N+1 per-message SELECTs.
    // For scope=me, batch the entire UPDATE into a single SQL statement.
    // For scope=all, run one UPDATE per id but reuse pre-fetched rows for auth checks.
    if (scope === 'me') {
      // Single bulk UPDATE — no per-row SELECT needed since SUPER_ADMIN guard is already verified above.
      try {
        await db.run(sql`
          UPDATE internal_messages
          SET hidden_for = CASE
      ...
      // scope=all: hard-delete each message (SUPER_ADMIN only — already enforced above).
      for (const id of ids) {
        try {
    ...
    // L-4 FIX: Wrap emit in try-catch — a socket error must not return 500 after successful deletes.
