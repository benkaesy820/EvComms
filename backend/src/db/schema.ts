...
export const users = sqliteTable('users', {
  ...
  // Admin subsidiary affinity — JSON array of subsidiary IDs this admin handles.
  // NULL / empty array = handles all subsidiaries (fallback generalist).
  subsidiaryIds: text('subsidiary_ids'),
...
export const conversations = sqliteTable('conversations', {
  ...
  // Ownership — one conversation per user (enforced by uq_conversations_user_id)
  ...
  // Denormalized counters for performance (#36)
  ...
  // Optional link to a registration report (when user registered with a report)
  registrationReportId: text('registration_report_id').references(() => registrationReports.id, { onDelete: 'set null' }),

  // Queue / workload tracking
  // waitingSince: set when a USER sends a message and no admin has replied yet; cleared on admin reply
  waitingSince: integer('waiting_since', { mode: 'timestamp' }),
  // lastAdminReplyAt: updated whenever an admin sends a message in this conversation
  ...
  // Archiving: list archived conversations
  archivedIdx: index('idx_conversations_archived').on(table.archivedAt, table.lastMessageAt)
    .where(sql`${table.archivedAt} IS NOT NULL`),

  // Archived by user (for audit)
  archivedByIdx: index('idx_conversations_archived_by').on(table.archivedBy),

  // Archived conversations (not archived)
  activeIdx: index('idx_conversations_active').on(table.lastMessageAt)
    .where(sql`${table.archivedAt} IS NULL AND ${table.deletedAt} IS NULL`),

  // Queue: conversations where user is waiting for admin reply, oldest first
...
export const messages = sqliteTable('messages', {
  ...
  // Content
  type: text('type', { enum: ['TEXT', 'IMAGE', 'DOCUMENT'] }).notNull(),
  ...
  // Reply reference — explicit return type breaks TypeScript circular inference
  replyToId: text('reply_to_id').references((): AnySQLiteColumn => messages.id, { onDelete: 'set null' }),

  // Media attachment (forward FK — same pattern as internalMessages/directMessages)
  ...
  // SECURITY FIX: Added FK constraint
  announcementId: text('announcement_id').references(() => announcements.id, { onDelete: 'set null' }),

  // Soft Delete
  deletedAt: integer('deleted_at', { mode: 'timestamp' }),
  // SECURITY FIX: Added ON DELETE action for consistency
...
export const media = sqliteTable('media', {
  ...
  cdnUrl: text('cdn_url'), // null for DOCUMENT type (requires presigned URL)
...
export const userStatusHistory = sqliteTable('user_status_history', {
  ...
  // Audit
  // SECURITY FIX: Added ON DELETE action to prevent orphaned records
  changedBy: text('changed_by').notNull().references(() => users.id, { onDelete: 'restrict' }),
  reason: text('reason'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
}, (table) => ({
...
export const announcements = sqliteTable('announcements', {
  ...
  // Public announcements for storefront
  publicActiveIdx: index('idx_announcements_public_active')
    .on(table.createdAt)
    .where(sql`${table.isActive} = 1 AND ${table.isPublic} = 1`),

  // Author's announcements
  createdByIdx: index('idx_announcements_created_by').on(table.createdBy),

  // SECURITY FIX: Index for expiresAt queries (filtering expired announcements)
  expiresAtIdx: index('idx_announcements_expires').on(table.expiresAt),
}))
...
export const internalMessages = sqliteTable('internal_messages', {
  ...
  // Content
  type: text('type', { enum: ['TEXT', 'IMAGE', 'DOCUMENT'] }).notNull().default('TEXT'),
  content: text('content'),
  mediaId: text('media_id').references(() => media.id, { onDelete: 'set null' }),
  replyToId: text('reply_to_id').references((): AnySQLiteColumn => internalMessages.id, { onDelete: 'set null' }),
...
// ============================================================================
// DM RECIPIENT STATUS — Track per-recipient read status for DMs
// ============================================================================
export const dmRecipientStatus = sqliteTable('dm_recipient_status', {
  id: text('id').primaryKey(), // ulid
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  // One record per user-partner pair
  uniqueStatus: unique('uq_dm_status').on(table.userId, table.partnerId),
  // Fast lookup for a user's DM status
  userIdx: index('idx_dm_status_user').on(table.userId),
  // Fast lookup for total unread across all DMs
  userUnreadIdx: index('idx_dm_status_unread').on(table.userId, table.unreadCount).where(sql`${table.unreadCount} > 0`),
}))

// ============================================================================
// INTERNAL MESSAGE READS — Track per-user last read for team chat
// ============================================================================
export const internalMessageReads = sqliteTable('internal_message_reads', {
  id: text('id').primaryKey(), // ulid
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  lastReadMessageId: text('last_read_message_id').references(() => internalMessages.id, { onDelete: 'set null' }),
  lastReadAt: integer('last_read_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
  unreadCount: integer('unread_count').notNull().default(0),
  updatedAt: integer('updated_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
}, (table) => ({
  // One record per user
  uniqueUser: unique('uq_internal_reads_user').on(table.userId),
  // Fast lookup for users with unread messages
  unreadIdx: index('idx_internal_reads_unread').on(table.unreadCount).where(sql`${table.unreadCount} > 0`),
}))
...
export const directMessages = sqliteTable('direct_messages', {
  ...
  content: text('content'),
  mediaId: text('media_id').references(() => media.id, { onDelete: 'set null' }),
  replyToId: text('reply_to_id').references((): AnySQLiteColumn => directMessages.id, { onDelete: 'set null' }),
  deletedAt: integer('deleted_at', { mode: 'timestamp' }),
  hiddenFor: text('hidden_for').notNull().default('[]'),
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().default(sql`(unixepoch())`),
}, (table) => ({
...
// ============================================================================
export const registrationReports = sqliteTable('registration_reports', {
  id: text('id').primaryKey(), // ulid
  userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  ...
  // Review tracking (consistent with userReports.resolvedBy)
  ...
  // Super Admin dashboard — pending reports sorted by newest
  statusCreatedIdx: index('idx_reg_reports_status_created').on(table.status, table.createdAt),

  // Lookup by user (e.g. on approval page)
  userIdx: index('idx_reg_reports_user').on(table.userId),

  // Lookup by reviewer
  reviewedByIdx: index('idx_reg_reports_reviewed_by').on(table.reviewedBy),
}))

// ============================================================================
// USER REPORTS TABLE (post-registration reports)
// ============================================================================
export const userReports = sqliteTable('user_reports', {
  ...
  // Link back to the source registration report (if this userReport was auto-created on approval)
  // NULL for manually-submitted post-registration reports
  sourceRegistrationReportId: text('source_registration_report_id').references(() => registrationReports.id, { onDelete: 'set null' }),
  ...
  // Admin dashboard — pending reports sorted by newest
  statusCreatedIdx: index('idx_user_reports_status_created').on(table.status, table.createdAt),

  // Lookup by user
  userIdx: index('idx_user_reports_user').on(table.userId),

  // Lookup by resolver
  resolvedByIdx: index('idx_user_reports_resolver').on(table.resolvedBy),
}))
...
export const usersRelations = relations(users, ({ many }) => ({
  ...
  registrationReports: many(registrationReports),
  userReports: many(userReports, { relationName: 'userReports' }),
  resolvedReports: many(userReports, { relationName: 'resolvedReports' }),
  assignedConversations: many(conversations, { relationName: 'assignedConversations' }),
  archivedConversations: many(conversations, { relationName: 'archivedConversations' }),
}))
...
export const conversationsRelations = relations(conversations, ({ one, many }) => ({
  ...
  archiver: one(users, { fields: [conversations.archivedBy], references: [users.id], relationName: 'archivedConversations' }),
  registrationReport: one(registrationReports, { fields: [conversations.registrationReportId], references: [registrationReports.id] }),
...
export const dmRecipientStatusRelations = relations(dmRecipientStatus, ({ one }) => ({
  user: one(users, { fields: [dmRecipientStatus.userId], references: [users.id] }),
  partner: one(users, { fields: [dmRecipientStatus.partnerId], references: [users.id] }),
}))

export const internalMessageReadsRelations = relations(internalMessageReads, ({ one }) => ({
  user: one(users, { fields: [internalMessageReads.userId], references: [users.id] }),
  lastReadMessage: one(internalMessages, { fields: [internalMessageReads.lastReadMessageId], references: [internalMessages.id] }),
}))

export const registrationReportsRelations = relations(registrationReports, ({ one, many }) => ({
  user: one(users, { fields: [registrationReports.userId], references: [users.id] }),
  media: one(media, { fields: [registrationReports.mediaId], references: [media.id] }),
  conversations: many(conversations),
}))

export const userReportsRelations = relations(userReports, ({ one }) => ({
  user: one(users, { fields: [userReports.userId], references: [users.id], relationName: 'userReports' }),
  media: one(media, { fields: [userReports.mediaId], references: [media.id] }),
  resolver: one(users, { fields: [userReports.resolvedBy], references: [users.id], relationName: 'resolvedReports' }),
}))
...
export type DMRecipientStatus = typeof dmRecipientStatus.$inferSelect
export type NewDMRecipientStatus = typeof dmRecipientStatus.$inferInsert

export type InternalMessageRead = typeof internalMessageReads.$inferSelect
export type NewInternalMessageRead = typeof internalMessageReads.$inferInsert

export type RegistrationReport = typeof registrationReports.$inferSelect
export type NewRegistrationReport = typeof registrationReports.$inferInsert
