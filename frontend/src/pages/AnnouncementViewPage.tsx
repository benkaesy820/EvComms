...
export function AnnouncementViewPage() {
  ...
  // Subscribe to real-time updates for this announcement (comments, reactions, votes)
  useAnnouncementSocket(id)
  ...
  // Regular admins can only edit their own announcements (backend enforces this too)
  const isOwnAnnouncement = announcement?.author?.id === user?.id
  const canEdit = isSuperAdmin || isOwnAnnouncement
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b shrink-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-muted-foreground" onClick={async () => {
            try {
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="max-w-3xl mx-auto w-full px-4 sm:px-6 -mt-8 relative pb-12">
          ...
          <article className="prose prose-sm dark:prose-invert max-w-none prose-p:leading-relaxed prose-headings:font-bold prose-a:text-primary hover:prose-a:text-primary/80 prose-img:rounded-xl">
            {/* SECURITY FIX: rehype-sanitize prevents XSS via malicious markdown */}
                ...
                return Object.entries(grouped).map(([emoji, { count, mine }]) => (
                  <button
                    ...
                    className={cn(
                      ...
                      mine
                        ? 'bg-primary/10 border-primary/30 text-primary font-medium cursor-pointer'
                        : 'bg-background text-muted-foreground hover:text-foreground hover:bg-accent cursor-pointer'
                    )}
                  >
                    <span>{emoji}</span>
            ...
            <div className="flex items-start gap-3 p-4 border-b">
              ...
              <div className="flex-1 relative">
                <textarea
                  ...
                  rows={2}
                  className="w-full resize-none rounded-xl border bg-muted/30 px-3 py-2.5 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                />
              </div>
              <Button
                size="icon"
                className="h-10 w-10 rounded-xl shrink-0 mt-0.5"
            ...
            <div className="divide-y">
              {commentsLoading ? (
                ...
                comments.map((comment) => {
                  ...
                  return (
                    <div key={comment.id} className="flex items-start gap-3 px-4 sm:px-5 py-4 group hover:bg-muted/20 transition-colors">
                      ...
                      {canDelete && (
                        <button
                          ...
                          className="opacity-60 sm:opacity-0 hover:opacity-100 group-hover:opacity-60 transition-opacity h-8 w-8 flex items-center justify-center rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive shrink-0 ml-auto"
