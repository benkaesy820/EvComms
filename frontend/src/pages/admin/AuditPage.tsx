...
import {
  ...
  Clock, Download, Settings, Image, Megaphone, Wrench, ChevronRight,
...
export function AuditPage() {
  ...
  const { data, isLoading, isError, hasNextPage, fetchNextPage, isFetchingNextPage } = useAuditLogs({
  ...
  const allLogs = useMemo(() => {
    ...
    // FIX #22: Apply client-side date range filter
    if (dateFrom) {
      const fromMs = new Date(dateFrom).getTime()
      filtered = filtered.filter(l => {
        const ts = typeof l.createdAt === 'number' ? (l.createdAt < 1e12 ? l.createdAt * 1000 : l.createdAt) : new Date(l.createdAt).getTime()
        return ts >= fromMs
      })
    }
    if (dateTo) {
      const toMs = new Date(dateTo).getTime() + 86400000 // inclusive end of day
      filtered = filtered.filter(l => {
        const ts = typeof l.createdAt === 'number' ? (l.createdAt < 1e12 ? l.createdAt * 1000 : l.createdAt) : new Date(l.createdAt).getTime()
        return ts <= toMs
      })
  ...
  }, [actionFilter, entityFilter, userSearch, dateFrom, dateTo])

  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="shrink-0 p-3 sm:p-4 border-b bg-background space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              onClick={() => {
                const csvContent = [
                  ['Timestamp', 'Action', 'Entity Type', 'Entity ID', 'User', 'Details'].join(','),
                  ...allLogs.map(log => [
                    format(parseTimestamp(log.createdAt), 'yyyy-MM-dd HH:mm:ss'),
                    `"${log.action}"`,
                    log.entityType,
                    log.entityId,
...
{isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 text-muted-foreground p-8 min-h-[300px]">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm mt-1">There was an error loading the audit logs. Please try again.</p>
                    </div>
                    <Button variant="outline" className="gap-2 rounded-xl" onClick={() => { queryClient.invalidateQueries({ queryKey: ['admin', 'audit-logs'] }) }}>
