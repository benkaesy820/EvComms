...
import {
  ...
  ScrollText, History, Info, LogOut, ImageIcon,
} from 'lucide-react'
...
export function UserDetailPage() {
  ...
  // FIX #14: Redirect early if userId param is missing — prevents hooks from running unscoped
  if (!userId) {
  ...
  const [openChatPending, setOpenChatPending] = useState(false)

  const updateMedia = useUpdateMediaPermission()
  const currentViewer = useAuthStore(s => s.user)
  const isSuperAdmin = currentViewer?.role === 'SUPER_ADMIN'
  const handleMediaToggle = () => {
    if (!user) return
    if (!user.mediaPermission && user.status !== 'APPROVED') {
      toast.error(`Cannot grant media permission to a ${user.status.toLowerCase()} user`)
      return
    }
    updateMedia.mutate({ userId: user.id, mediaPermission: !user.mediaPermission })
  }
  ...
  const handleOpenChat = async () => {
    if (!userId || openChatPending) return
    setOpenChatPending(true)
    try {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden min-h-0 bg-muted/10">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto w-full pb-[calc(1rem+env(safe-area-inset-bottom)+5rem)] sm:pb-8">
            ...
            <div className="lg:col-span-4 xl:col-span-3 space-y-4 lg:sticky lg:top-8">
              <div className="rounded-2xl border bg-card/50 backdrop-blur overflow-hidden shadow-sm">
                ...
                <div className="p-3 space-y-1.5 flex flex-col bg-muted/10">
                  <Button variant="outline" className="w-full justify-start gap-2 shadow-sm rounded-xl h-10" onClick={handleOpenChat} disabled={openChatPending}>
                    {openChatPending ? <LeafLogo className="h-4 w-4 animate-spin" /> : <MessageSquare className="h-4 w-4 text-primary" />}Message User
                  </Button>
                  ...
                  {isSuperAdmin ? (
                    <Button variant="ghost" className="w-full justify-start gap-2 text-muted-foreground rounded-xl h-9" disabled={revokeSessions.isPending} onClick={() => revokeSessions.mutate(user.id)}>
                      {revokeSessions.isPending ? <LeafLogo className="h-4 w-4 animate-spin" /> : <LogOut className="h-4 w-4" />}
                      Revoke All Sessions
                    </Button>
                  ) : (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="w-full">
                          <Button variant="ghost" className="w-full justify-start gap-2 text-muted-foreground/40 rounded-xl h-9 cursor-not-allowed" disabled>
                            <LogOut className="h-4 w-4" />
                            Revoke All Sessions
                            <span className="ml-auto text-[10px] text-muted-foreground/40">SA only</span>
                          </Button>
                        </div>
                      </TooltipTrigger>
                      <TooltipContent side="left" className="max-w-[180px] text-center">
                        Only Super Admins can revoke user sessions
                      </TooltipContent>
                    </Tooltip>
                  )}
                </div>
              ...
              <Tabs defaultValue="overview" className="flex flex-col gap-6">
                ...
                <TabsContent value="overview" className="mt-0">
                  ...
                  <div className="rounded-2xl border bg-card/50 backdrop-blur overflow-hidden shadow-sm mt-6">
                    ...
                    <div className="flex flex-col">
                      <InfoRow icon={ImageIcon} label="Media Uploads">
                        <div className="flex items-center gap-3">
                          ...
                          <Switch
                            ...
                            disabled={updateMedia.isPending || (!user.mediaPermission && user.status !== 'APPROVED')}
                          />
                        </div>
                      </InfoRow>
                          ...
                          return (
                            <div key={h.id} className="flex items-start gap-4 rounded-xl border bg-background/50 p-4 transition-colors hover:bg-accent/30">
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="font-semibold text-sm">{h.previousStatus} <ArrowLeft className="inline h-3 w-3 mx-1 text-muted-foreground rotate-180" /> {h.newStatus}</span>
                        ...
                        {auditLogs.map((log) => (
                          <div key={log.id} className="rounded-xl border bg-background/50 p-4 transition-colors hover:bg-accent/30">
                            <div className="flex items-center justify-between gap-4 mb-2">
                              <span className="font-semibold text-sm tracking-tight">{formatAction(log.action)}</span>
                              <Badge variant="secondary" className="text-[10px] shrink-0 font-medium">{log.entityType}</Badge>
                            </div>
                            {log.details && (() => {
                              const parsed = parseDetails(log.details)
                              return parsed ? (
                                <div className="mt-2 rounded-lg bg-muted/50 px-3 py-2 border-l-2 border-primary/40 space-y-0.5">
                                  {Object.entries(parsed).map(([k, v]) => (
                                    <DetailRow key={k} label={k} value={v} />
                                  ))}
                                </div>
                              ) : (
                                <div className="mt-2 rounded-lg bg-muted/20 p-2 text-xs font-mono text-muted-foreground break-all border-l-2 border-primary/40">
                                  {log.details}
                                </div>
                              )
                            })()}
      ...
      <Dialog open={statusDialog} onOpenChange={setStatusDialog}>
        <DialogContent className="sm:max-w-[440px]">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleStatusSubmit} disabled={
