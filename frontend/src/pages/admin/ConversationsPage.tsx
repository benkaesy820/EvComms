...
type AssignVars = { conversationId: string; adminId: string | null; admin?: { id: string; name: string; role: any } | null }
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (data: { message: Message }) => {
      ...
      updateCache((old) => {
        ...
        return { ...old, pages: [{ ...first, messages: [data.message, ...first.messages] }, ...old.pages.slice(1)] }
      })
            ...
            <div key={msg.id}>
              <MessageBubble
                ...
                onRetry={(msg as any).status === 'FAILED' ? () => {
                  ...
                  sendMessage.mutate({ type: msg.type, content: msg.content ?? undefined, mediaId: (msg.media as any)?.id, replyToId: msg.replyToId ?? undefined })
                } : undefined}
              />
            </div>
          )}
...
export function ConversationsPage() {
  ...
  useEffect(() => {
    ...
    const handleConvUpdated = (upd: { conversationId: string; lastMessageAt?: number | null; lastMessage?: Message | null; unreadCount?: number; adminUnreadCount?: number; assignedAdminId?: string | null; waitingSince?: number | null }) => {
      const found = queryClient.getQueriesData<{ pages: Array<{ conversations: Conversation[] }> }>({ queryKey: ['conversations'] })
        .some(([, data]) => data?.pages.some(p => p.conversations.some(c => c.id === upd.conversationId)))
