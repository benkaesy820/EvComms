...
import {
  ...
  Calendar, Clock, MoreVertical, Pencil, ExternalLink, Info,
} from 'lucide-react'
...
import {
  useAnnouncements, useDeleteAnnouncement, useVoteAnnouncement, useAnnouncementSocket,
...
function AnnouncementCard({
  ...
  return (
    <div
      className={cn(
        'group border-b border-border/40 bg-transparent overflow-hidden transition-all hover:bg-accent/30 cursor-pointer',
        !announcement.isActive && 'opacity-50',
        isOwn && 'ring-1 ring-inset ring-primary/10',
      )}
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-3 left-4 right-4">
                <div className="flex items-center gap-2">
                  ...
                  {isOwn && (
                    <Badge className="text-[10px] bg-primary/80 text-primary-foreground border-none">
                      Your post
                    </Badge>
                  )}
                </div>
          ...
          <div className="p-4">
            ...
            {/* Content preview */}
            <div className="text-sm text-muted-foreground mt-1.5 leading-relaxed line-clamp-3 prose prose-sm dark:prose-invert prose-p:my-0 prose-headings:my-0">
              ...
              <div className="shrink-0 self-start sm:self-auto ml-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-foreground shrink-0 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
...
export function AnnouncementsPage() {
  ...
  // Subscribe to real-time announcement events (votes, reactions, comments) for all announcements
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="p-4 border-b shrink-0">
        ...
        {/* Role-scoped info banner for regular admins */}
        {!isSuperAdmin && (
          <div className="mt-3 flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-3 py-2">
            ...
            <p className="text-[11px] text-amber-700 dark:text-amber-400 leading-relaxed">
              You can create and edit announcements. Only Super Admins can delete announcements created by others.
            </p>
          </div>
        )}
      </div>
      ...
      <ScrollArea className="flex-1 bg-background">
        <div className="flex flex-col pb-4">
          {items.length === 0 ? (
            ...
            items.map((ann) => {
              const isOwn = ann.author?.id === currentUser?.id
              // Delete is allowed if: super admin (can delete any), or own announcement
      ...
      <AlertDialog open={!!deleteConfirmId} onOpenChange={(open) => { if (!open) setDeleteConfirmId(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes the announcement for all users. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
