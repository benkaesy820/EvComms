...
import {
  ...
  Eye, UserCog, MessageSquare, AlertTriangle, ArrowLeft, Info,
} from 'lucide-react'
...
import { conversations as convApi, adminStats } from '@/lib/api'
    ...
    >
      ...
      <div className="flex items-center gap-2 shrink-0 self-center ml-auto md:ml-0" onClick={e => e.stopPropagation()}>
        {user.role === 'USER' && (
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-1.5 sm:gap-2 rounded-lg bg-muted/50 px-2 sm:px-3 py-1.5 sm:py-2 border">
                <ImageIcon className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                <span className="text-[10px] text-muted-foreground font-medium hidden sm:inline">Media</span>
                <Switch
...
const { data, isLoading, isError, hasNextPage, fetchNextPage, isFetchingNextPage } = useAdminUsers({
 status: filterStatus,
 role: 'USER',
 search: debouncedSearch || undefined,
  ...
  // Server-accurate counts for filter chips — avoids showing "Pending (12)" when 200 exist
  // but only the first page has loaded. Falls back to local count if stats unavailable.
  const { data: statsData } = useQuery({
    queryKey: ['admin', 'stats'],
    queryFn: () => adminStats.get(),
    staleTime: 0,
  })
  const serverPending   = statsData?.stats?.scope === 'super_admin' ? (statsData.stats.users?.pending   ?? null) : null
  const serverApproved  = statsData?.stats?.scope === 'super_admin' ? (statsData.stats.users?.approved  ?? null) : null
  const serverSuspended = statsData?.stats?.scope === 'super_admin' ? (statsData.stats.users?.suspended ?? null) : null

  const { allUsers, localPending, localApproved, localSuspended } = useMemo(() => {
    ...
    return { allUsers: flat, localPending: pending, localApproved: approved, localSuspended: suspended }
  }, [data])

  const pendingCount   = serverPending   ?? localPending
  const approvedCount  = serverApproved  ?? localApproved
  const suspendedCount = serverSuspended ?? localSuspended
  ...
  const openChatInFlight = useRef<Set<string>>(new Set())
  const handleOpenChat = useCallback(async (userId: string) => {
    if (openChatInFlight.current.has(userId)) return
    openChatInFlight.current.add(userId)
    try {
      ...
      openChatInFlight.current.delete(userId)
    }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="p-3 sm:p-4 lg:p-5 border-b space-y-3 sm:space-y-4">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          {pendingCount > 0 && (
            <Badge variant="secondary" className="gap-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border-amber-200 dark:border-amber-800 shrink-0">
              <Clock className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">{pendingCount} pending review</span>
              <span className="sm:hidden">{pendingCount} pending</span>
            </Badge>
          )}
        </div>

        {/* Scope banner for regular admins */}
        {!isSuperAdmin && (
          <div className="flex items-start gap-2 rounded-lg bg-muted/50 border px-3 py-2">
            <Info className="h-3.5 w-3.5 text-muted-foreground shrink-0 mt-0.5" />
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Showing users assigned to your conversations. Super Admins can see all users.
            </p>
          </div>
        )}
...
{isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center gap-4 text-muted-foreground p-8 min-h-[300px]">
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-destructive/10">
                  <AlertTriangle className="h-8 w-8 text-destructive/60" />
                </div>
                <div className="text-center">
                  <p className="font-semibold text-foreground">Failed to load users</p>
                  <p className="text-sm mt-1">There was an error loading the user list. Please try again.</p>
                </div>
                <Button variant="outline" className="gap-2 rounded-xl" onClick={() => { queryClient.invalidateQueries({ queryKey: ['admin', 'users'] }) }}>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  className={cn(
                    ...
                    (newStatus === 'REJECTED' || newStatus === 'SUSPENDED') && !reason.trim()
