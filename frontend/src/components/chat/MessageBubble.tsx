...
function StatusIcon({ status, isSending, groupMode, groupSize, readCount }: {
  ...
  if (groupMode) {
    ...
    if (status === 'READ' || allRead) return <CheckCheck className="h-3.5 w-3.5 text-blue-500" />
...
function MessageBubbleInner({
  message, hideAvatar, bubbleClassName, onReply, onReact, onDelete, onRetry, canDeleteOverride,
  ...
  const senderLabel = senderRole !== 'USER' ? `${senderName} (${senderRole === 'SUPER_ADMIN' ? 'Super Admin' : 'Admin'})` : senderName
  ...
  return (
    <>
      ...
      >
        ...
        <div className={cn('flex items-end gap-1.5 max-w-[75%]', isMine ? 'flex-row-reverse' : 'flex-row')}>
          ...
          <div className={cn('flex flex-col relative', isMine ? 'items-end' : 'items-start min-w-0 flex-1')}>
            ...
            <div className={cn(
              ...
              isMine
                ? 'bg-[#d9fdd3] dark:bg-[#005c4b] text-foreground rounded-tr-none border border-black/5 dark:border-white/5'
                : 'bg-card text-card-foreground rounded-tl-none border border-black/5 dark:border-white/5',
              isTemp && 'opacity-70',
        ...
        {/* Outgoing avatar */}
        {isMine && (
          <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/20 text-[11px] font-bold text-primary mb-0.5 select-none', hideAvatar && 'invisible')}>
            {initials}
          </div>
        )}
      </div>
