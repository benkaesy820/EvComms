...
import {
  ...
  ThumbsUp, ThumbsDown, X, Users, ExternalLink,
} from 'lucide-react'
...
function BannerTemplate({ announcement, onVote, onView }: { announcement: Announcement; onVote: (id: string, vote: 'UP' | 'DOWN') => void; onView: (id: string) => void }) {
    ...
    <div className={cn('relative overflow-hidden rounded-lg border', style.bg, style.border)}>
      ...
      <div className="flex items-start gap-3 px-4 py-3 pl-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div
            className="text-xs text-muted-foreground mt-0.5 line-clamp-2 leading-relaxed prose prose-sm dark:prose-invert prose-p:my-0 cursor-pointer"
          ...
          <div className="flex items-center gap-3 mt-2">
            <div className="flex items-center gap-0.5">
              <button
                onClick={(e) => { e.stopPropagation(); onVote(announcement.id, 'UP') }}
                className={cn(
                ...
                aria-label={`Upvote announcement - ${announcement.upvoteCount} upvotes`}
                aria-pressed={announcement.userVote === 'UP'}
              >
                <ThumbsUp className="h-3 w-3" aria-hidden="true" />
                <span className="tabular-nums">{announcement.upvoteCount}</span>
              </button>
              <button
                onClick={(e) => { e.stopPropagation(); onVote(announcement.id, 'DOWN') }}
                className={cn(
                  'flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] transition-colors',
                ...
                aria-label={`Downvote announcement - ${announcement.downvoteCount} downvotes`}
                aria-pressed={announcement.userVote === 'DOWN'}
              >
...
function CardTemplate({ announcement, onVote, onView }: { announcement: Announcement; onVote: (id: string, vote: 'UP' | 'DOWN') => void; onView: (id: string) => void }) {
  const style = TYPE_STYLE[announcement.type]
  const Icon = style.icon
  ...
  return (
    <div className={cn('rounded-xl border p-4 shadow-sm', style.bg, style.border)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold cursor-pointer hover:text-primary transition-colors" onClick={() => onView(announcement.id)}>{announcement.title}</h4>
          <div
            className="text-xs text-muted-foreground mt-1 leading-relaxed line-clamp-3 prose prose-sm dark:prose-invert prose-p:my-0 cursor-pointer"
            onClick={() => onView(announcement.id)}
          >
            <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]} components={inlineComponents}>
              {announcement.content}
...
function MinimalTemplate({ announcement, onVote, onView }: { announcement: Announcement; onVote: (id: string, vote: 'UP' | 'DOWN') => void; onView: (id: string) => void }) {
  const style = TYPE_STYLE[announcement.type]
  const Icon = style.icon
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-lg border bg-muted/30 px-3 py-2">
      ...
      <span className="text-xs font-medium truncate flex-1 cursor-pointer hover:text-primary transition-colors" onClick={() => onView(announcement.id)}>{announcement.title}</span>
...
function AnnouncementItem({ announcement, onVote, onView }: { announcement: Announcement; onVote: (id: string, vote: 'UP' | 'DOWN') => void; onView: (id: string) => void }) {
  switch (announcement.template) {
    case 'CARD':
      return <CardTemplate announcement={announcement} onVote={onVote} onView={onView} />
    case 'MINIMAL':
      return <MinimalTemplate announcement={announcement} onVote={onVote} onView={onView} />
    case 'BANNER':
    case 'DEFAULT':
    default:
      return <BannerTemplate announcement={announcement} onVote={onVote} onView={onView} />
  }
}
        ...
        <div className="px-4 pb-3 space-y-2 max-h-[280px] overflow-y-auto">
          {visibleAnnouncements.map((ann) => (
            <div key={ann.id} className="relative group">
              <AnnouncementItem announcement={ann} onVote={handleVote} onView={handleView} />
              <Button
                variant="ghost"
                size="icon"
