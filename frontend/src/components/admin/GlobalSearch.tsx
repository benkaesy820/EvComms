...
import { Search, Users, MessageSquare, Megaphone, X, CheckCircle, UserCog, ArrowRight, FileText, Flag } from 'lucide-react'
import { cn, getInitials } from '@/lib/utils'
import { adminSearch, adminUsers, type SearchResults, type SearchType } from '@/lib/api'
...
export function GlobalSearch() {
  ...
  const results: SearchResults = {
    ...
    registrationReports: data?.registrationReports ?? [],
    userReports: data?.userReports ?? [],
  }

  const hasResults =
    results.users.length + results.conversations.length +
    results.announcements.length + results.messages.length +
    results.registrationReports.length + results.userReports.length > 0

  // go MUST be declared before flatResults — flatResults useMemo closes over it,
  // and const bindings are NOT hoisted. Declaring it after caused the crash:
  // "Cannot access 'go' before initialization"
  ...
  const flatResults = useMemo(() => {
    ...
    results.users.forEach(u => items.push({ type: 'user', id: u.id, onSelect: () => go(u.role === 'USER' ? `/admin/users/${u.id}` : (isSuperAdmin ? `/admin/admins` : `/admin/users/${u.id}`)) }))
    results.conversations.forEach(c => items.push({ type: 'conversation', id: c.id, onSelect: () => go(`/admin?conversationId=${c.id}`) }))
    results.announcements.forEach(a => items.push({ type: 'announcement', id: a.id, onSelect: () => go(`/admin/announcements/${a.id}`) }))
    results.messages.forEach(m => items.push({ type: 'message', id: m.id, onSelect: () => go(`/admin?conversationId=${m.conversationId}`) }))
    results.registrationReports.forEach(r => items.push({ type: 'regReport', id: r.id, onSelect: () => go('/admin/reports') }))
    results.userReports.forEach(r => items.push({ type: 'userReport', id: r.id, onSelect: () => go('/admin/user-reports') }))
    return items
  }, [results.users, results.conversations, results.announcements, results.messages, results.registrationReports, results.userReports, go, isSuperAdmin])
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        if (selectedIndex >= 0 && selectedIndex < flatResults.length) {
          ...
          flatResults[0].onSelect()
        }
      }
    }
    document.addEventListener('keydown', handler)
    return () => document.removeEventListener('keydown', handler)
  }, [open, hasResults, flatResults, selectedIndex])
  ...
  const trigger = (
    <button
      ...
      className="flex items-center gap-2 h-9 px-2 sm:px-3 rounded-xl border border-border/40 bg-accent/20 text-muted-foreground text-[13px] hover:bg-accent/40 shadow-sm transition-all w-full max-w-[160px] sm:max-w-[200px] md:max-w-[260px]"
  ...
  const palette = open ? createPortal(
    <div
      className="fixed inset-0 z-[9999] flex items-start justify-center pt-[5vh] sm:pt-[8vh] px-2 sm:px-4"
      onClick={close}
    >
      {/* Backdrop */}
      ...
      >
        ...
        <div className="max-h-[60vh] sm:max-h-[50vh] overflow-y-auto overscroll-contain">
          {!debouncedQ ? (
            ...
            let idx = -1
            return (
            <div ref={resultsRef} className="divide-y divide-border/50">
              {/* Users */}
                ...
                <div>
                  ...
                  {results.users.map((u) => {
                    ...
                    <button
                      key={u.id}
                      className={cn(
                        "flex items-center gap-3 px-4 py-3 w-full text-left group transition-colors",
                        selectedIndex === idx ? "bg-accent" : "hover:bg-accent"
                      )}
                      onClick={() => go(u.role === 'USER' ? `/admin/users/${u.id}` : (isSuperAdmin ? `/admin/admins` : `/admin/users/${u.id}`))}
                ...
                <div>
                  ...
                  {results.conversations.map((c) => {
                    ...
                    <button
                      ...
                      aria-label={`Open conversation with ${c.user.name}`}
                    >
                ...
                <div>
                  ...
                  {results.announcements.map((a) => {
                    ...
                    <button
                      ...
                      onClick={() => go(`/admin/announcements/${a.id}`)}
                ...
                <div>
                  ...
                  {results.messages.map((m) => {
                    ...
                    <button
                      ...
                      aria-label={`View message from ${m.sender.name}`}
                    >
              ...
              {/* Registration Reports (super admin only) */}
              {results.registrationReports.length > 0 && (
                <div>
                  <p className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground bg-muted/30 sticky top-0">
                    Registration Reports
                  </p>
                  {results.registrationReports.map((r) => {
                    idx++
                    return (
                    <button
                      key={r.id}
                      ...
                      aria-label={`View registration report: ${r.subject}`}
                    >
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400">
                        <Flag className="h-4 w-4" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{highlight(r.subject, debouncedQ)}</p>
                        <p className="text-xs text-muted-foreground truncate">{r.user.name} · {r.user.email}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={cn('text-[10px] font-medium px-1.5 py-0.5 rounded-full', r.status === 'PENDING' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-muted text-muted-foreground')}>
                          {r.status}
                        </span>
                        <ArrowRight className="h-3.5 w-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
                      </div>
              ...
              {/* User Reports */}
              {results.userReports.length > 0 && (
                <div>
                  <p className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground bg-muted/30 sticky top-0">
                    User Reports
                  </p>
                  {results.userReports.map((r) => {
                    idx++
                    return (
                    <button
                      key={r.id}
                      ...
                      onClick={() => go(`/admin/user-reports`)}
                      aria-label={`View user report: ${r.subject}`}
                    >
